                ...
                case "POST":
                    switch (e.Path)
                    {
                        ...
                        case "test/app/bin":
                            ...
                            break;
                        case "Recipes":
                            byte[] Rpayloadraw = (byte[])e.Data;
                            string Rpayload = System.Text.Encoding.UTF8.GetString(Rpayloadraw, 0, Rpayloadraw.Length);
                            e.res.ContentType = "application/json ";
                            e.res.setContent(Rpayload);
                           // IList<RecipeDatar> recipes = new List<SqlMotor>();
                           // IList<RecipeData> recipes = JsonConvert.DeserializeObject<IList<RecipeData>>(Rpayload);

                           // MsSqlLogic msSqlLogic = new MsSqlLogic();
                           // msSqlLogic.InsertDataIntoTable(Rpayload);
                            //Log.Info(Recipes);
                            res = 1;
                            break;
                        case "test/error":
